...
#alias dps="docker ps --format '{{.ID}}|{{.Names}}|{{.Image}}|{{.Status}}|{{.Ports}}' | awk -F  \"|\" 'function da(id){ \"docker image inspect \$(docker container inspect \" id \" --format \\\"{{.Image}}\\\") --format \\\"{{.Architecture}}\\\"\" | getline a; return a }{split(\$3,name,\"/\");print \$1\"|\"\$2\"|\"da(\$1)\"|\"name[2]\"|\"\$4\"|\"\$5}' | column -t -s '|' "
